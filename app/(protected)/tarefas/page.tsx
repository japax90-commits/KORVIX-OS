...
export default async function TarefasPage(){
 ...
 const [{data:tasks,error},{data:profile}]=await Promise.all([supabase.from("tasks").select("id,title,assigned_to_name,priority,status,due_date,related_entity_type,related_entity_label").order("due_date",{ascending:true}),supabase.from("users").select("name").eq("id",user.id).maybeSingle()]);
 ...
 {header:"Prazo",cell:t=>formatDate(t.due_date),hideOnMobile:true},{header:"Status",cell:t=><StatusBadge status={t.status}/>}];
 return <div className="space-y-6"><div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><div><h2 className="text-xl font-semibold tracking-tight text-ink-900">Tarefas</h2><p className="text-sm text-ink-500">Sistema transversal operacional.</p></div><CreateTaskButton userName={profile?.name??user.email??"Usuário"}/></div><Card><CardHeader title="Todas as tarefas" subtitle="Dados reais do Supabase"/><DataTable columns={columns} rows={rows} emptyLabel="Nenhuma tarefa cadastrada. Use Nova tarefa para começar."/></Card></div>;
}
